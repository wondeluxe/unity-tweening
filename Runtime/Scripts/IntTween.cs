...
{
	/// <summary>
	/// Lightweight structure for handling a tween between two int values.
	/// </summary>
	/// <remarks>
	/// <see cref="IntTween"/> provides efficient logic for tweening between two int values at the expense of features provided by <see cref="Tween"/>.
	/// Clients are expected to manage the update cycle of <see cref="IntTween"/> and apply updates to target objects where required.
	/// </remarks>

	[Serializable]
	public struct IntTween
	{
		[SerializeField]
		...
		/// <summary>
		/// Initialize a new IntTween ready for updating.
		/// </summary>
		...
		public IntTween(int from, int to, float duration, AnimationCurve curve)
		{
			this.from = from;
			this.to = to;
		...
		public int Value
		{
			get => from + (int)((to - from) * curve.Evaluate(NormalizedTime));
		}
		...
		/// <summary>
		/// Set the <see cref="IntTween"/> with new values and reset <see cref="Time"/> to 0.
		/// </summary>
		/// <param name="from">Start value; value to tween from.</param>
		/// <param name="to">End value; value to tween to.</param>
		/// <param name="duration">Time (unit agnostic) the tween will take to complete.</param>
		...
		public void Set(int from, int to, float duration, AnimationCurve curve)
		{
			this.from = from;
			this.to = to;
			this.duration = Mathf.Max(duration, 0f);
		...
		{
			return $"(From = {from}, To = {to}, Duration = {duration:G})";
		}
	}
}