...
{
	/// <summary>
	/// Lightweight structure for handling a tween between two Vector4 values.
	/// </summary>
	/// <remarks>
	/// <see cref="Vector4Tween"/> provides efficient logic for tweening between two Vector4 values at the expense of features provided by <see cref="Tween"/>.
	/// Clients are expected to manage the update cycle of <see cref="Vector4Tween"/> and apply updates to target objects where required.
	/// </remarks>

	[Serializable]
	public struct Vector4Tween
	{
		[SerializeField]
		...
		/// <summary>
		/// Initialize a new Vector4Tween ready for updating.
		/// </summary>
		...
		public Vector4Tween(Vector4 from, Vector4 to, float duration, AnimationCurve curve)
		{
			this.from = from;
			this.to = to;
		...
		{
			get => Vector4.Lerp(from, to, curve.Evaluate(NormalizedTime));
		}
		...
		/// <summary>
		/// Set the <see cref="Vector4Tween"/> with new values and reset <see cref="Time"/> to 0.
		/// </summary>
		/// <param name="from">Start value; value to tween from.</param>
		/// <param name="to">End value; value to tween to.</param>
		/// <param name="duration">Time (unit agnostic) the tween will take to complete.</param>
		...
		public void Set(Vector4 from, Vector4 to, float duration, AnimationCurve curve)
		{
			this.from = from;
			this.to = to;
			this.duration = Mathf.Max(duration, 0f);
