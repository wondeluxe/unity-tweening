...
{
	/// <summary>
	/// Lightweight structure for handling a tween between two double values.
	/// </summary>
	/// <remarks>
	/// <see cref="DoubleTween"/> provides efficient logic for tweening between two double values at the expense of features provided by <see cref="Tween"/>.
	/// Clients are expected to manage the update cycle of <see cref="DoubleTween"/> and apply updates to target objects where required.
	/// </remarks>

	[Serializable]
	public struct DoubleTween
	{
		[SerializeField]
		...
		/// <summary>
		/// Initialize a new DoubleTween ready for updating.
		/// </summary>
		...
		public DoubleTween(double from, double to, float duration, AnimationCurve curve)
		{
			this.from = from;
			this.to = to;
		...
		public double Value
		{
			get => from + (to - from) * curve.Evaluate(NormalizedTime);
		}
		...
		/// <summary>
		/// Set the <see cref="DoubleTween"/> with new values and reset <see cref="Time"/> to 0.
		/// </summary>
		/// <param name="from">Start value; value to tween from.</param>
		/// <param name="to">End value; value to tween to.</param>
		/// <param name="duration">Time (unit agnostic) the tween will take to complete.</param>
		...
		public void Set(double from, double to, float duration, AnimationCurve curve)
		{
			this.from = from;
			this.to = to;
			this.duration = Mathf.Max(duration, 0f);
