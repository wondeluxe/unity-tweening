...
{
	/// <summary>
	/// Lightweight structure for handling a tween between two float values.
	/// </summary>
	/// <remarks>
	/// <see cref="FloatTween"/> provides efficient logic for tweening between two float values at the expense of features provided by <see cref="Tween"/>.
	/// Clients are expected to manage the update cycle of <see cref="FloatTween"/> and apply updates to target objects where required.
	/// </remarks>

	[Serializable]
	public struct FloatTween
	{
		[SerializeField]
		...
		/// <summary>
		/// Initialize a new FloatTween ready for updating.
		/// </summary>
		...
		public FloatTween(float from, float to, float duration, AnimationCurve curve)
		{
			this.from = from;
			this.to = to;
		...
		/// <summary>
		/// Set the <see cref="FloatTween"/> with new values and reset <see cref="Time"/> to 0.
		/// </summary>
		/// <param name="from">Start value; value to tween from.</param>
		/// <param name="to">End value; value to tween to.</param>
		/// <param name="duration">Time (unit agnostic) the tween will take to complete.</param>
		...
		public void Set(float from, float to, float duration, AnimationCurve curve)
		{
			this.from = from;
			this.to = to;
			this.duration = Mathf.Max(duration, 0f);
