...
{
	/// <summary>
	/// Lightweight structure for handling a tween between two Color values.
	/// </summary>
	/// <remarks>
	/// <see cref="ColorTween"/> provides efficient logic for tweening between two Color values at the expense of features provided by <see cref="Tween"/>.
	/// Clients are expected to manage the update cycle of <see cref="ColorTween"/> and apply updates to target objects where required.
	/// </remarks>

	[Serializable]
	public struct ColorTween
	{
		[SerializeField]
		...
		/// <summary>
		/// Initialize a new ColorTween ready for updating.
		/// </summary>
		...
		public ColorTween(Color from, Color to, float duration, AnimationCurve curve)
		{
			this.from = from;
			this.to = to;
		...
		public Color Value
		{
			get => Color.Lerp(from, to, curve.Evaluate(NormalizedTime));
		}
		...
		/// <summary>
		/// Set the <see cref="ColorTween"/> with new values and reset <see cref="Time"/> to 0.
		/// </summary>
		/// <param name="from">Start value; value to tween from.</param>
		/// <param name="to">End value; value to tween to.</param>
		/// <param name="duration">Time (unit agnostic) the tween will take to complete.</param>
		...
		public void Set(Color from, Color to, float duration, AnimationCurve curve)
		{
			this.from = from;
			this.to = to;
			this.duration = Mathf.Max(duration, 0f);
