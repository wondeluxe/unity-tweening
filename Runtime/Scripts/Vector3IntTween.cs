...
{
	/// <summary>
	/// Lightweight structure for handling a tween between two Vector3Int values.
	/// </summary>
	/// <remarks>
	/// <see cref="Vector3IntTween"/> provides efficient logic for tweening between two Vector3Int values at the expense of features provided by <see cref="Tween"/>.
	/// Clients are expected to manage the update cycle of <see cref="Vector3IntTween"/> and apply updates to target objects where required.
	/// </remarks>

	[Serializable]
	public struct Vector3IntTween
	{
		[SerializeField]
		...
		/// <summary>
		/// Initialize a new Vector3IntTween ready for updating.
		/// </summary>
		...
		public Vector3IntTween(Vector3Int from, Vector3Int to, float duration, AnimationCurve curve)
		{
			this.from = from;
			this.to = to;
		...
		{
			get => Vector3IntExtensions.Lerp(from, to, curve.Evaluate(NormalizedTime));
		}
		...
		/// <summary>
		/// Set the <see cref="Vector3IntTween"/> with new values and reset <see cref="Time"/> to 0.
		/// </summary>
		/// <param name="from">Start value; value to tween from.</param>
		/// <param name="to">End value; value to tween to.</param>
		/// <param name="duration">Time (unit agnostic) the tween will take to complete.</param>
		...
		public void Set(Vector3Int from, Vector3Int to, float duration, AnimationCurve curve)
		{
			this.from = from;
			this.to = to;
			this.duration = Mathf.Max(duration, 0f);
